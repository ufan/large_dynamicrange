...
The scintillation photons are uniformly distributed in space.
However, due to the refraction on the boundary, only \SI{22.5}{\percent} of them are within the total reflection angle and will undergo the transmission process to either end of the bar.
...
$\varepsilon_3$ ($\approx$0.95) is the transmission coefficient of EJ-560 which is the coupling material between the readout device and the bar,
and $\varepsilon_4$ ($\approx$0.15) is the mean quantum efficiency of the readout PMT.

PSD adopts R4443 as the readout device, which is a 10-stage PMT. 
...
\label{eq:dygain}
\end{equation}
According to Eq.~\ref{eq:dygain}, the 8th dynode (Dy8) of R4443 has a gain of $4.71\times10^4$ and has a positive charge output \SI{362} {\femto\coulomb} for one MIP signal. 
It's a little larger than our design value of 300 fC, and the difference can be eliminated easily by the fine adjustment of the PMT supply high voltage. 
To cover the required upper dynamic range, the 5th dynode (Dy5) is used to the larger signals measurements, as the maximum relative gain factor across three dynodes is \SI{63} from a numerical calculation. 
Therefore, the total readout system could achieve the demanded wide range of 4 orders, and also provide a cross calibration between this two dynodes.
