...
Fig.~\ref{fig:mip} shows the raw ADC spectrum of the Dy8 channel from the right-side PMT, with the MIP and the noise spectrum in the same figure.
...
As MPV is proportional to the mean light output, Fig.~\ref{fig:attenuation} represents the light attenuation effect of the PSD bar and the data points can be fitted using the following model~\cite{taiuti_measurement_1996}:
\begin{equation}
...
$\eta_{max}$ and $\eta_{min}$ can then be used to calculate the maximum and the minimum light output of the PSD bar.
...
The data from the left-side PMT is processed in the same way, and the $dy58$s from both sides are listed in Table~\ref{tab:mip}.
