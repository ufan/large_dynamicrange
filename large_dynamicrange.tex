...
To determine the appropriate dynode stages for readout, an estimation of the mean number of photon electrons(PEs) generated by one MIP in the middle of the bar is carried out.
\begin{align}
...
R4443 is a 10-stage PMT with a gain of $1\times10^6$ at typical supply voltage using standard voltage divider(i.e. same voltage drop between dynodes).
A simple calculation shows that the multiplication ratio between adjacent dynodes is about 4.
Based on PEs from Eq.~\ref{eq:pes}, the best-suited dynode composition is dynode5$+$dynode8, which corresponds to a MIP response of \SI{450}{\femto\coulomb} and gain ratio of 65.
The values are little higher than expected, but the supply voltage can be adjusted to get the optimal result. 

\subsection{Base circuit design and implementation}
Based on the previous discussion, a voltage divider is designed to extract current pulses from dynode5 and dynode8, as shown in Fig.\ref{fig:divider}.
