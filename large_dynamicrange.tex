...
These functions impose harsh demands for the readout system of PSD, which, together with the PMT, needs to cover a wide range of signal amplitudes from singly charged minimum ionizing particles(MIPs) to relativistic calcium ions, while in the same time keeping sufficient resolution and large signal-to-noise ratio(SNR) for effective particle identification.   
...
\subsection{Light output of EJ-200}
According to the Bethe-Bloch equation~\cite{olive_review_2014}, the energy deposited in PSD bars is approximately proportional to $Z^2$ for charged particles heavier than electron.
However, the light yield of organic scintillators is a nonlinear function of $\Delta E$ due to the quenching effect.
This effect can be described by the semi-emprical Birks equation~\cite{birks_theory_2013} in the low energy region.
In the relativistic region, the An expand Birks-Chou model,BTV model, EDSE model

Despite the theoretical difficulty, various expriments have been carried out to investigate the response of scintillator.
These experiments give us a direct reference for design.
Our estimation is based on the beam test result of AMS, which uses the same kind of plastic scintillator.
