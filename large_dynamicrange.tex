...
Due to the theoretical difficulties metioned above, the direct beam test result of AMS-02 TOF prototype~\cite{marrocchesi2011beam}, which also uses EJ-200 as the detection material and has the same thickness, will be referenced for our light yield estimation.
According to ~\cite{marrocchesi2011beam}, the light output of calcium is about 270 times larger than that of proton.
As all singly charged minimum ionization particles have the same response in the plastic scintillator, this result indicates a maximum of 270 times difference in the light yield for all the particle species covered by PSD.
...
To make things clear, we define the mean light collected by the end PMT of PSD bars when a singly charged minimum ionizing particle penetrates through the center of the bar vertically as one MIP, and use it as the unit of dynamic range calculation.

The first contribution comes from the broad field of view DAMPE possesses, which implies a maximum incidence angle of \SI{60}{\degree} for particles through PSD.
This doubles the energy deposit in PSD, and thus expand the maximum light yield to \SI{540}{MIPs}.  
Secondly, the long length of PSD's plastic scintillator bar makes the light attenuation non-negligible.
The bars are wrapped by the Tyvek paper to improve transmission efficiency, and a preliminary test shows that the 
At last, the statistical nature of ionizing process and the finite resolution of detector components 
