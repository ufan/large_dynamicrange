...
Fig.~\ref{fig:mip} shows the raw ADC spectrum of the Dy8 channel from the right-side PMT, with the MIP and the pedestal in the same figure.
It can be seen the MIP peak is clearly separated from the pedestal.
The MIP spectrum follows the Landau distribution and the most probable value (MPV) of this distribution equals to \SI{1}{MIP} as defined in Sec.~\ref{sec:requirement}.
The MPV ($\mu_{mpv}$) and the resolution ($\sigma_{mpv}$) of the distribution can be extracted by fitting the lower half of the MIP peak using the Gaussian distribution, as shown in Fig.~\ref{fig:mip}.
The fitting results from both sides are summarized in Table~\ref{tab:mip}, together with the standard deviation of the pedestals ($\sigma_{noise}$).
Thus, the lower limit of the dynamic range can be calculated as $5\sigma_{noise}/\mu_{mpv}$, which gives \SI{0.073}{MIPs} and \SI{0.054}{MIPs} for the right-side and left-side respectively.
...
The MIP measurements were then carried out every \SI{10}{\centi\meter} along the bar to investigate the light attenuation effect in detail.
The MPV values at each position was extracted 
The relative light output at each position is shown in Fig.~\ref{fig:attenuation}.
The light attenuation in the long bar can be described precisely by the following model~\cite{taiuti_measurement_1996}:
\begin{equation}
...
The relative gain between Dy8 and Dy5 can also be extracted from the cosmic ray test by merging the data from all tested positions.
Fig.~\ref{fig:dy58} shows the correlation between the ADC counts of Dy5 and Dy8 from the right side of the bar.
It can be seen that a good linearity has been achieved between the two dynode stages.
By fitting the data with a linear function, the relative gain between Dy8 and Dy5, $dy58$, can be extracted as the slope of the fitting function as shown in Fig.~\ref{fig:dy58}.
The data from the left-side PMT can be processed in the same way, and the $dy58$s from both sides are listed in Table~\ref{tab:mip}.

Thus the upper limit of this readout unit can be calculated, and it's about  \SI{1462}{MIPs} for the right side.
...
It can calculated that the relative resolution of the PSD detector module for Ar is about \SI{4.5}{\percent} and \SI{4.2}{\percent}.
 ...
 Combining the results from the cosmic ray test (see Table~\ref{tab:mip}), the light yield of $Ar$ in the bar can be calculated as ${\mu_{Ar}\cdot dy58} / {\mu_{mpv}}$, which gives \SI{220}{MIPs} and \SI{216}{MIPs} for the left and right side of the bar respectively.
