...
The dynamic range of the PSD readout unit is affected by many different factors.
To simplify the work, we define the mean light yield produced by a minimum ionizing particle (electron with $\beta\gamma$ = 5 for example) penetrating vertically through the center of the PSD bar as one MIP, and use it as the unit for the dynamic range estimation.
...
The light yield of organic scintillator is a nonlinear function of the energy loss $dE/dx$ due to the quenching effect, which can be described by the Birks' law~\cite{birks_theory_2013}.
 For heavy ions in the relativistic region, the scintillation mechanism is much more complicated and the Birks' law fails to apply directly. 
