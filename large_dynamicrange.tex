...
We adopt the readout scheme of exacting signals from two dynodes of R4443 and then connecting them to separate VA160 channels.
...
While alternative methods exist~\cite{katayose2008development,kampert1994high,genolini_low_2003}, this readout scheme is preferred because the largest SNR can be achieved without adding extra electronic components, which in turn implies robustness and lower power consumption.
To reduce the crosstalk effect, signals from the two dynodes shall be connected to different VA160 chips as shown in Fig.\ref{fig:readout_scheme}.

\subsection{Selection of the dynode stages}
% \subsection{Photon number of one MIP}
FEE board based on VA160 has been developed and fully tested~\cite{fee}.
Outputs from VA160 are digitized by a 14 bits ADC, and an effective linear range up to \SI{12}{\pico\coulomb} can be achieved for a single channel.  
When connected to the PSD detector module, the mean RMS of electronic noise is about \SI{4}{\femto\coulomb}. 
Assuming the maximum noise level is \SI{50}{\percent} larger than the mean value, i.e.\SI{6}{\femto\coulomb} and 5$\sigma$ is needed to separate electronic noise and signal, the lower limit 
